...
def turkey_today():
    # Turkey uses UTC+3 year-round. Using the server's UTC date directly can
    # show the wrong attendance day around midnight, so calculate the date
    # from UTC+3 explicitly.
...
def ctx(): return {"current_user":user(),"timedelta":timedelta}
...
def home():
    ...
    with db() as c:
        ...
        members=c.execute("SELECT id,username,profile_photo,profile_mime FROM users WHERE role='member' ORDER BY username").fetchall()
        attendance=c.execute(
            "SELECT day,present FROM attendance WHERE user_id=%s ORDER BY day DESC",
            (u["id"],)
        ).fetchall()
    return render_template("index.html",weeks=weeks,projects=projects,members=members,
...
@app.route("/api/faziletcodeai", methods=["POST"])
@login_req
def faziletcodeai_api():
    data=request.get_json(silent=True) or {}
    question=(data.get("question") or "").strip()
    code=(data.get("code") or "")
    # UI/backend contract is ready. A real model provider should be connected
    # here using a server-side environment variable; never expose an API key
    # in HTML/JavaScript.
    if not question:
        return jsonify({"answer":"Sorunu yazarsan yardımcı olabilirim."})
    return jsonify({"answer":"FaziletCodeAI arayüzü hazır. Gerçek yapay zekâ modelini güvenli biçimde Render sunucusundaki API anahtarıyla bağlayacağız. Gönderdiğin kod sunucuya ulaştı; model bağlantısı tamamlandığında burada gerçek analiz görünecek."})
...
def attendance_mark():
    u=user()
    if u["role"]=="admin":
        flash("Admin için yoklama üye panelinden yönetilir.")
        return redirect(url_for("admin"))
    selected=request.form.get("day","")
    try:
        day=date.fromisoformat(selected)
    except ValueError:
        abort(400)
    today=turkey_today()
    # A member may only mark the current Turkey-local day.
    if day != today:
        flash("Sadece bugünün yoklaması işaretlenebilir.")
        return redirect(url_for("home"))
    with db() as c:
        c.execute("""INSERT INTO attendance(user_id,day,present)
                     VALUES(%s,%s,TRUE)
                     ON CONFLICT(user_id,day)
                     DO UPDATE SET present=EXCLUDED.present""",(u["id"],day))
    flash("Bugünkü yoklaman işaretlendi. Yarın 00:00'dan sonra yeni gün aktif olacak.")
    return redirect(url_for("home"))
