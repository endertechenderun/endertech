...
def init_db():
    with db() as conn:
        with conn.cursor() as c:
            ...
            if c.execute("SELECT COUNT(*) AS n FROM weeks").fetchone()["n"] == 0:
...
def user():
    if not session.get("uid"): 
        return None
    with db() as c:
        return c.execute("SELECT * FROM users WHERE id=%s", (session["uid"],)).fetchone()
...
def login():
    if request.method == "POST":
        with db() as c:
            u = c.execute("SELECT * FROM users WHERE username=%s", (request.form["username"].strip(),)).fetchone()
    ...
    if request.method == "POST":
        with db() as c:
            if request.form["action"] == "photo":
                ...
                if not f or f.mimetype not in {"image/png", "image/jpeg", "image/webp"}: 
                    flash("PNG/JPG/WEBP seç.")
                elif len(f.read()) > 2 * 1024 * 1024: 
                    flash("Fotoğraf en fazla 2 MB.")
                else:
                    f.stream.seek(0)
                    c.execute("UPDATE users SET profile_photo=%s, profile_mime=%s WHERE id=%s", (f.read(), f.mimetype, u["id"]))
                ...
                if u["role"] == "member" and check_password_hash(full["password_hash"], request.form["old"]) and request.form["new"] == request.form["confirm"] and len(request.form["new"]) >= 4:
                    c.execute("UPDATE users SET password_hash=%s WHERE id=%s", (generate_password_hash(request.form["new"]), u["id"]))
                    flash("Şifre değiştirildi.")
                else: 
                    flash("Şifre bilgileri hatalı.")
...
def photo(uid):
    with db() as c:
        x = c.execute("SELECT profile_photo, profile_mime FROM users WHERE id=%s", (uid,)).fetchone()
...
def new_project():
    if request.method == "POST":
        with db() as c:
            c.execute("INSERT INTO projects(user_id, title, html_code, python_code) VALUES(%s, %s, %s, %s)", 
                      (user()["id"], request.form["title"], request.form["html"], request.form["python"]))
        return redirect(url_for("home"))
...
def edit_project(pid):
    with db() as c:
        p = c.execute("SELECT * FROM projects WHERE id=%s", (pid,)).fetchone()
    if not p or p["user_id"] != user()["id"]:
        abort(403)
    if request.method == "POST":
        with db() as c:
            c.execute("UPDATE projects SET title=%s, html_code=%s, python_code=%s, updated_at=CURRENT_TIMESTAMP WHERE id=%s", 
                      (request.form["title"], request.form["html"], request.form["python"], pid))
        return redirect(url_for("home"))
...
def member():
    try:
        with db() as c:
            c.execute("INSERT INTO users(username, password_hash, role) VALUES(%s, %s, 'member')", 
...
def reset(uid):
    with db() as c:
        c.execute("UPDATE users SET password_hash=%s WHERE id=%s AND role='member'", 
...
def delete(uid):
    with db() as c:
        c.execute("DELETE FROM users WHERE id=%s AND role='member'", (uid,))
    return redirect(url_for("admin"))
...
def week():
    with db() as c:
        c.execute("INSERT INTO weeks(title, description) VALUES(%s, %s)", 
...
def att():
    with db() as c:
        c.execute("""INSERT INTO attendance(user_id, day, present) VALUES(%s, %s, %s)
        ON CONFLICT(user_id, day) DO UPDATE SET present=EXCLUDED.present""", 
        (request.form["uid"], request.form["day"], request.form["present"] == "1"))
    return redirect(url_for("admin"))
